...
static uint32_t host_import(void*o,const char*n,uint32_t t){uint32_t h=2166136261u;harness*ctx=(harness*)o;if(strstr(n,"_entry")||strstr(n,"_ctor")||strstr(n,"_fini")||strstr(n,"_dt_init")||strstr(n,"_dt_fini")||!strcmp(n,"c_func")||!strcmp(n,"b_func")||!strcmp(n,"c_data")||!strcmp(n,"a_data_pointer")||!strcmp(n,"a_weak_pointer")||!strcmp(n,"a_weak_value")||!strcmp(n,"optional_value")||!strcmp(n,"trace_event")||!strcmp(n,"trace_reset")||!strcmp(n,"trace_events")||!strcmp(n,"definitely_missing_symbol")||!strcmp(n,"missing_dependency"))return 0;while(*n){h^=(unsigned char)*n++;h*=16777619u;}uint32_t address=0x07000000u|(h&0x000ffffcu);if(t==2){const uint32_t return_zero[]={0xe3a00000u,0xe12fff1eu};if(arm_interp_load(ctx->cpu,address,(const uint8_t*)return_zero,sizeof(return_zero))||arm_interp_set_page_permissions(ctx->cpu,address&~0xfffu,0x1000,5))return 0;}return address;}
...
int main(int argc,char**argv){
  ...
  uint32_t base_constructors=agr_constructor_count(r),base_finalizers=agr_finalizer_count(r);
  for(int i=0;i<1000;i++){uint32_t stress=agr_dlopen(r,"libagr_A.so");if(!stress||stress!=a1||agr_dlclose(r,stress)||agr_constructor_count(r)!=base_constructors||agr_finalizer_count(r)!=base_finalizers){fprintf(stderr,"load/unload stress failed at %d: %s\n",i,agr_dlerror(r));return 8;}}
