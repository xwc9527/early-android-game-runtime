...
static void events(get_fn count,get_fn get){int i,n=count(0);putchar('[');for(i=0;i<n;i++)printf("%s%d",i?",":"",get(i));putchar(']');}
int main(void){
  int i,j,first,reload,thread_values[10]={0};
  ...
  printf("{");
  for(i=0;i<10;i++){int r=((run_fn)sym(suite,symbols[i]))();printf("%s\"%s\":%d",i?",":"",names[i],r);if(i>=5&&i<=7){printf(",\"%s_events\":",names[i]);events(count,get);}if(i==9)for(j=0;j<10;j++)thread_values[j]=value(20+j);}
  first=((run_fn)sym(cross,"agr_eh2b_cross"))();printf(",\"cross\":%d,\"cross_events\":",first);events(count,get);printf(",\"cross_base\":%d,\"cross_derived\":%d",value(10),value(11));
  if(dlclose(cross))return 4; cross=open_lib("libagr_eh2b_A.so"); reload=((run_fn)sym(cross,"agr_eh2b_cross"))();
  printf(",\"reload_cross\":%d,\"thread_one\":%d,\"thread_two\":%d,\"thread_ret_one\":%d,\"thread_ret_two\":%d,\"thread_id_one\":%d,\"thread_id_two\":%d,\"globals_one\":%d,\"globals_two\":%d,\"globals_after_one\":%d,\"globals_after_two\":%d}\n",reload,thread_values[0],thread_values[1],thread_values[2],thread_values[3],thread_values[4],thread_values[5],thread_values[6],thread_values[7],thread_values[8],thread_values[9]);
