...
int main(void){
  int i,j,first,reload,thread_values[4]={0};
  ...
  printf("{");
  for(i=0;i<10;i++){int r=((run_fn)sym(suite,symbols[i]))();printf("%s\"%s\":%d",i?",":"",names[i],r);if(i>=5&&i<=7){printf(",\"%s_events\":",names[i]);events(count,get);}if(i==9)for(j=0;j<4;j++)thread_values[j]=value(20+j);}
