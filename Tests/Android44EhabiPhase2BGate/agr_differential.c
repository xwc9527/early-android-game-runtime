...
static const char *const forbidden_host[] = {
    ...
    "__cxa_allocate_exception","__cxa_free_exception","__cxa_throw","__cxa_type_match",
    "__cxa_begin_catch","__cxa_end_catch","__cxa_rethrow","__cxa_get_globals",
    "__cxa_get_globals_fast","__cxa_current_exception_type","__cxa_begin_cleanup",
    "__cxa_end_cleanup",NULL
};
...
static int read_events(harness*h,uint32_t probe,int*out,uint32_t cap){uint32_t c=agr_dlsym(h->runtime,probe,"agr_eh2b_count"),g=agr_dlsym(h->runtime,probe,"agr_eh2b_get");int32_t n=0;if(!c||!g||call_guest(h,c,NULL,0,&n)||n<0||(uint32_t)n>cap)return -1;for(int32_t i=0;i<n;i++){uint32_t a=(uint32_t)i;int32_t v;if(call_guest(h,g,&a,1,&v))return -1;out[i]=v;}return n;}
...
static void dump_failure(harness*h,const char*stage){
    fprintf(stderr,"EHABI2B_FAILURE stage=%s error=%s pc=%08x lr=%08x sp=%08x cpsr=%08x\n",stage,h->error,
...
static int run_symbol(harness*h,uint32_t object,const char*symbol,int32_t*out){uint32_t fn=agr_dlsym(h->runtime,object,symbol);if(!fn){snprintf(h->error,sizeof(h->error),"missing %s",symbol);return -1;}return call_guest(h,fn,NULL,0,out);}

int agr_ehabi2b_run(int argc,char**argv){
    if(argc!=8){fprintf(stderr,"usage: %s gnustl probe types suite C B A\n",argv[0]);return 2;}
    ...
    uint32_t gnustl=0,probe=0,types_h=0,suite=0,c_h=0,b_h=0,a_h=0;
    if(load(&h,"libgnustl_shared.so",argv[1],0x01000000,&gnustl)||load(&h,"libagr_eh2b_probe.so",argv[2],0x03000000,&probe)||load(&h,"libagr_eh2b_types.so",argv[3],0x04000000,&types_h)||load(&h,"libagr_eh2b_suite.so",argv[4],0x05000000,&suite)||load(&h,"libagr_eh2b_C.so",argv[5],0x06000000,&c_h)||load(&h,"libagr_eh2b_B.so",argv[6],0x07000000,&b_h)||load(&h,"libagr_eh2b_A.so",argv[7],0x08000000,&a_h)){fprintf(stderr,"%s\n",h.error);return 5;}
    const char*watched[]={"__cxa_allocate_exception","__cxa_free_exception","__cxa_throw","__cxa_type_match","__cxa_begin_catch","__cxa_end_catch","__cxa_rethrow","__cxa_get_globals","__cxa_get_globals_fast","__cxa_current_exception_type","_Unwind_RaiseException","_Unwind_Resume","__gxx_personality_v0"};
    for(uint32_t i=0;i<sizeof(watched)/sizeof(watched[0]);i++)watch_addr(&h,gnustl,watched[i],watched[i]);
    int32_t typed=0,inheritance=0,multiple=0,pointer=0,rethrow=0,lref=0,lvalue=0,lrethrow=0,nested=0,cross_ok=0,reload_ok=0;
    int ref_events[64],value_events[64],rethrow_events[64],cross_events[64];int ref_n=0,value_n=0,rethrow_n=0,cross_n=0;
    if(run_symbol(&h,suite,"agr_eh2b_typed",&typed)||run_symbol(&h,suite,"agr_eh2b_inheritance",&inheritance)||run_symbol(&h,suite,"agr_eh2b_multiple",&multiple)||run_symbol(&h,suite,"agr_eh2b_pointer",&pointer)||run_symbol(&h,suite,"agr_eh2b_rethrow",&rethrow)||run_symbol(&h,suite,"agr_eh2b_lifetime_ref",&lref)||(ref_n=read_events(&h,probe,ref_events,64))<0||run_symbol(&h,suite,"agr_eh2b_lifetime_value",&lvalue)||(value_n=read_events(&h,probe,value_events,64))<0||run_symbol(&h,suite,"agr_eh2b_lifetime_rethrow",&lrethrow)||(rethrow_n=read_events(&h,probe,rethrow_events,64))<0||run_symbol(&h,suite,"agr_eh2b_nested",&nested)){dump_failure(&h,"suite");return 6;}
    uint32_t cross_fn=agr_dlsym(h.runtime,a_h,"agr_eh2b_cross");if(!cross_fn||call_guest(&h,cross_fn,NULL,0,&cross_ok)||(cross_n=read_events(&h,probe,cross_events,64))<0){dump_failure(&h,"cross");return 7;}
    uint32_t c_pc=agr_dlsym(h.runtime,c_h,"agr_eh2b_C_throw"),b_pc=agr_dlsym(h.runtime,b_h,"agr_eh2b_B_call"),a_pc=cross_fn;
    if(h.exidx_mismatches){fprintf(stderr,"formal linker exidx ownership mismatches: %u\n",h.exidx_mismatches);return 8;}
    if(agr_dlclose(h.runtime,a_h)||agr_dlclose(h.runtime,b_h)||agr_dlclose(h.runtime,c_h)){fprintf(stderr,"unload: %s\n",agr_dlerror(h.runtime));return 9;}
    ...
    a_h=agr_dlopen(h.runtime,"libagr_eh2b_A.so");if(!a_h){fprintf(stderr,"reload A: %s\n",agr_dlerror(h.runtime));return 11;}
    cross_fn=agr_dlsym(h.runtime,a_h,"agr_eh2b_cross");if(!cross_fn||call_guest(&h,cross_fn,NULL,0,&reload_ok)){fprintf(stderr,"reload cross: %s\n",h.error);return 12;}
    ...
    agr_module_info rtti_owner={0};uint32_t rtti=agr_dlsym(h.runtime,types_h,"_ZTI7Derived");int rtti_owned=rtti&&agr_find_module(h.runtime,rtti,&rtti_owner)==0;
    printf("{\"typed\":%d,\"inheritance\":%d,\"multiple\":%d,\"pointer\":%d,\"rethrow\":%d,\"lifetime_ref\":%d,\"lifetime_ref_events\":",typed,inheritance,multiple,pointer,rethrow,lref);print_events(ref_events,ref_n);printf(",\"lifetime_value\":%d,\"lifetime_value_events\":",lvalue);print_events(value_events,value_n);printf(",\"lifetime_rethrow\":%d,\"lifetime_rethrow_events\":",lrethrow);print_events(rethrow_events,rethrow_n);printf(",\"nested\":%d,\"cross\":%d,\"cross_events\":",nested,cross_ok);print_events(cross_events,cross_n);printf(",\"reload_cross\":%d,\"unload_stale\":%u,\"reload_owned\":%u,\"exidx_mismatches\":%u,\"rtti_owned\":%d,\"rtti_owner\":\"%s\",\"trace\":",reload_ok,stale_count,reload_owned,h.exidx_mismatches,rtti_owned,rtti_owned?rtti_owner.name:"");print_trace(&h);printf(",\"exidx\":");print_exidx(&h);printf("}\n");
    agr_runtime_destroy(h.runtime);arm_interp_destroy(h.cpu);return 0;
}

#if !defined(AGR_EHABI2B_EMBEDDED)
int main(int argc,char**argv){return agr_ehabi2b_run(argc,argv);}
#endif
