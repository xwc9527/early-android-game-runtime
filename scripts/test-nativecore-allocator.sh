...
clang++ -std=gnu++98 -fno-exceptions -fno-rtti "${SAN[@]}" "${INC[@]}" -c "$ROOT/Runtime/Bionic/agr_futex_host.cpp" -o "$OBJ/futex.o"
clang++ -std=gnu++98 -fno-exceptions -fno-rtti "${SAN[@]}" "${INC[@]}" -c "$ROOT/Runtime/Bionic/agr_bionic_sync.cpp" -o "$OBJ/sync.o"
clang++ -std=gnu++98 -fno-exceptions -fno-rtti "${SAN[@]}" "${INC[@]}" -c "$ROOT/Runtime/Bionic/agr_bionic_tls.cpp" -o "$OBJ/tls.o"
clang++ -std=gnu++98 -fno-exceptions -fno-rtti "${SAN[@]}" "${INC[@]}" -c "$ROOT/Runtime/Bionic/agr_bionic_errno_host.cpp" -o "$OBJ/errno.o"
clang++ -std=gnu++98 -fno-exceptions -fno-rtti "${SAN[@]}" "${INC[@]}" -c "$ROOT/Runtime/Bionic/agr_bionic_thread_lifecycle.cpp" -o "$OBJ/lifecycle.o"
clang++ "${SAN[@]}" "$OBJ/test.o" "$OBJ/runtime.o" "$OBJ/vma.o" "$OBJ/host-services.o" "$OBJ/thread-attr.o" "$OBJ/linker.o" "$OBJ/dynamic.o" "$OBJ/mmap.o" "$OBJ/futex.o" "$OBJ/sync.o" "$OBJ/tls.o" "$OBJ/errno.o" "$OBJ/lifecycle.o" -pthread -lm -framework CoreFoundation -framework Security -o "$OUT"
"$OUT"
