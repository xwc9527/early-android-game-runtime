...
static int32_t mem_read_cb(void *user, uint32_t address, void *data, uint32_t size) {
    return arm_interp_read(((agr_guest *)user)->cpu, address, (uint8_t *)data, size);
}
...
static void write_u32(agr_guest *g, uint32_t address, uint32_t value) {
    arm_interp_write(g->cpu, address, (const uint8_t *)&value, 4);
}
static uint32_t read_u32(agr_guest *g, uint32_t address) {
    uint32_t value = 0; arm_interp_read(g->cpu, address, (uint8_t *)&value, 4); return value;
}
static int read_guest_string(agr_guest *g, uint32_t address, char *text, uint32_t capacity) {
    if (!address || !capacity) return -1;
    for (uint32_t i=0; i<capacity; i++) {
        if (arm_interp_read(g->cpu,address+i,(uint8_t *)&text[i],1)) return -1;
        if (!text[i]) return 0;
    }
    text[capacity-1]=0; return -1;
}
static uint32_t argument(agr_guest *g, uint32_t index) {
    if (index < 4) return arm_interp_get_reg(g->cpu, index);
    return read_u32(g, arm_interp_get_reg(g->cpu, 13) + (index - 4) * 4);
}
...
static void guest_return(agr_guest *g, uint32_t value, uint32_t value_r1) {
    uint32_t target = arm_interp_get_reg(g->cpu, 14);
    uint32_t cpsr = arm_interp_get_cpsr(g->cpu);
    arm_interp_set_reg(g->cpu, 0, value);
    arm_interp_set_reg(g->cpu, 1, value_r1);
    arm_interp_set_cpsr(g->cpu, (target & 1) ? (cpsr | 0x20) : (cpsr & ~0x20));
    arm_interp_set_reg(g->cpu, 15, target & ~1u);
}
...
static int dispatch_jni(agr_guest *g, uint32_t address) {
    ...
    if (slot == 186 || slot == 187 || slot == 189 || slot == 222) {
        ...
        if (argument(g, 2)) { uint8_t one = 1; arm_interp_write(g->cpu, argument(g, 2), &one, 1); }
...
static void apply_client_array(agr_guest *g, client_array *a, uint32_t vertices,
    ...
    if (size) arm_interp_read(g->cpu, a->pointer, (uint8_t *)buffer, (uint32_t)size);
...
static int dispatch_egl(agr_guest *g, const char *name) {
    ...
    if (!strcmp(name, "eglSwapBuffers")) {
        ...
        if (g->yield_after_swap && g->cooperative_thread_active) {
            for (uint32_t i = 0; i < 16; i++) g->parked_regs[i] = arm_interp_get_reg(g->cpu, i);
            g->parked_cpsr = arm_interp_get_cpsr(g->cpu);
...
static int dispatch_graphics(agr_guest *g, const char *name) {
    ...
    if (!strcmp(name,"glMultMatrixf")) { GLfloat values[16]; arm_interp_read(g->cpu,argument(g,0),(uint8_t *)values,sizeof(values)); glMultMatrixf(values); guest_return(g,0,0); return 1; }
    ...
    if (!strcmp(name, "glDrawArrays") || !strcmp(name, "glDrawElements")) {
        ...
        if (!strcmp(name, "glDrawArrays")) { first = argument(g, 1); count = argument(g, 2); vertices = first + count; }
            ...
            indices = malloc(count * isz); arm_interp_read(g->cpu, argument(g, 3), (uint8_t *)indices, (uint32_t)(count * isz));
...
static int dispatch_import(agr_guest *g, const char *name) {
    ...
    if (!strcmp(name, "AConfiguration_getLanguage") || !strcmp(name, "AConfiguration_getCountry")) {
        ...
        arm_interp_write(g->cpu, argument(g, 1), (const uint8_t *)value, 2);
        guest_return(g, 0, 0); return 1;
    }
    ...
    if (!strcmp(name, "AAssetManager_open")) {
        ...
        do { if (i + 1 >= sizeof(path) || arm_interp_read(g->cpu, address+i, (uint8_t *)&path[i], 1)) {
    ...
    if (!strcmp(name, "AAsset_read") || !strcmp(name, "AAsset_close") ||
        ...
        while (done < count) {
            ...
            if (!got) break;
            if (arm_interp_write(g->cpu,argument(g, 1)+done,buffer,(uint32_t)got)) {
    ...
    if (agr_dispatch_system(g->runtime, name, regs, arm_interp_get_reg(g->cpu, 13), &out) == 0 && out.handled) {
        ...
        if (out.action == AGR_ACTION_CALL_ONCE) {
            ...
            for (uint32_t i = 0; i < 16; i++) arm_interp_set_reg(g->cpu, i, saved[i]);
            arm_interp_set_cpsr(g->cpu, saved_cpsr);
            ...
            if (g->parked_valid) {
                ...
                uint32_t prior = agr_current_thread(g->runtime);
                agr_set_current_thread(g->runtime, g->pending_thread_id);
                for (uint32_t i = 0; i < 16; i++) arm_interp_set_reg(g->cpu, i, g->parked_regs[i]);
                arm_interp_set_cpsr(g->cpu, g->parked_cpsr);
                g->parked_valid = 0; g->cooperative_thread_active = 1; g->call_depth++;
                int resumed = run_until_return(g);
                g->call_depth--; g->cooperative_thread_active = 0;
                agr_set_current_thread(g->runtime, prior);
                for (uint32_t i = 0; i < 16; i++) arm_interp_set_reg(g->cpu, i, saved[i]);
                arm_interp_set_cpsr(g->cpu, saved_cpsr);
            ...
            if (agr_dispatch_system(g->runtime, "pthread_mutex_lock", lock_args,
                                    arm_interp_get_reg(g->cpu, 13), &lock) || !lock.handled) {
...
static int run_until_return(agr_guest *g) {
    for (;;) {
        ...
        uint32_t address = arm_interp_get_reg(g->cpu, 15) - 4;
...
static int32_t call_address(agr_guest *g, uint32_t target, const uint32_t *args, uint32_t count, int32_t *result) {
    if (g->call_depth >= 16) { set_error(g, "guest nested call depth exceeded"); return -1; }
    ...
    g->call_depth--;
    if (rc < 0) return -1; if (result) *result = (int32_t)arm_interp_get_reg(g->cpu, 0); return 0;
}
...
int32_t agr_guest_read(agr_guest *g, uint32_t address, void *bytes, uint32_t size) {
    return g ? arm_interp_read(g->cpu, address, (uint8_t *)bytes, size) : -1;
}
int32_t agr_guest_write(agr_guest *g, uint32_t address, const void *bytes, uint32_t size) {
    return g ? arm_interp_write(g->cpu, address, (const uint8_t *)bytes, size) : -1;
}
...
int32_t agr_guest_create_gles1_pbuffer(agr_guest *g, int width, int height) {
    ...
    EGLint da[] = {EGL_PLATFORM_ANGLE_TYPE_ANGLE, EGL_PLATFORM_ANGLE_TYPE_METAL_ANGLE, EGL_NONE};
    g->display = getPlatformDisplay ? getPlatformDisplay(EGL_PLATFORM_ANGLE_ANGLE, EGL_DEFAULT_DISPLAY, da) : EGL_NO_DISPLAY;
    EGLint major, minor; if (!g->display || !eglInitialize(g->display, &major, &minor)) { set_error(g, "ANGLE Metal eglInitialize failed"); return -1; }
...
uint64_t agr_guest_instruction_count(agr_guest *g) { return g->instruction_count; }
