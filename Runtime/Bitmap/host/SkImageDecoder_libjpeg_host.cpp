...
#include "SkRTConf.h"
#include "SkRect.h"
/* Host: SkCanvas omitted; fill_below_level writes pixels directly. */
...
static void fill_below_level(int y, SkBitmap* bitmap) {
    /* Host adaptation of KitKat fill_below_level without linking SkCanvas.
       Remaining rows of a partial progressive JPEG are filled white. */
    if (!bitmap || !bitmap->getPixels() || y >= bitmap->height()) return;
    const int width = bitmap->width();
    const int height = bitmap->height();
    const size_t rb = bitmap->rowBytes();
    uint8_t* base = static_cast<uint8_t*>(bitmap->getPixels());
    for (int row = y; row < height; ++row) {
        uint32_t* dst = reinterpret_cast<uint32_t*>(base + row * rb);
        for (int x = 0; x < width; ++x) {
            dst[x] = SkPackARGB32(0xFF, 0xFF, 0xFF, 0xFF);
        }
    }
}
