...
Integer records six real crossings as `UNRESOLVED` and does not write `PREREQUISITE_CLOSED`. `valueOf` stops at `Dalvik.ClassInitialization`, `Dalvik.ObjectAllocation`, `Dalvik.MethodInvocation`, and `Dalvik.StaticFieldArrayRoots`. `Framework.ZygotePreload` stays in the Integer closure and stops at `Dalvik.ClassInitialization` and `Libcore.BootClassLoading`. `Dalvik.StaticFieldArrayRoots` keeps source status `SOURCE_CLOSED` while its relationship stays `UNRESOLVED`. The Integer cluster remains `SOURCE_LOCATED` with `MIGRATION_AUTHORIZED=false`. The Resources index and seed are unchanged.
