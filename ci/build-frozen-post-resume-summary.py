#!/usr/bin/env python3
"""Build exact-commit closure evidence for Frozen Bubble post-resume Phase 1."""
import argparse,hashlib,importlib.util,json,os,pathlib,subprocess

ROOT=pathlib.Path(__file__).resolve().parents[1]
TARGET="Frozen Bubble Post-Resume Compatibility Phase 1"
def git(*args): return subprocess.check_output(["git",*args],cwd=ROOT,text=True).strip()
def load(path): return json.loads((ROOT/path).read_text(encoding="utf-8"))

def main():
    p=argparse.ArgumentParser(); p.add_argument("--result",required=True)
    p.add_argument("--output",default="build/artifacts/frozen-post-resume-run-summary.json")
    a=p.parse_args(); result=load(a.result); semantic_path="ci/governance/semantic-diffs/frozen-bubble-post-resume.json"
    ...
    target_pass=bool(contract.get("passed") is True and result.get("launch_result")==0 and
        ...
        after.get("last_method")=="Landroid/app/Activity;->onPostResumeV" and
        result.get("classification")=="post_resume_complete_no_followup_event" and
    ...
    summary={
      ...
      "target":{"name":TARGET,"status":"pass" if target_pass else "fail",
        "stage":"post_resume_complete" if target_pass else "post_resume_blocked",
        "normalized_signature":"" if target_pass else "framework:activity_post_resume:closure_failed",
        ...
        "requirements":{"synthetic_contract":contract.get("passed") is True,
          ...
          "last_method":after.get("last_method"),"next_boundary_recorded":result.get("classification")}},
      "coverage_delta":{"new_imports":[],"new_android_api":[],"new_jni":[],"new_lifecycle":["Activity.onPostResume"]},
      "behavior_delta":["generic APK launch completes API19 Activity.performResume through onPostResume"],
