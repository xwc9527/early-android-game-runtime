#!/usr/bin/env python3
"""Build exact-commit closure evidence for Framework Runtime Continuation Phase 1."""
...
def main():
    ...
    p.add_argument("--output",default="build/artifacts/framework-continuation-run-summary.json")
    a=p.parse_args(); result=load(a.result); semantic_path="ci/governance/semantic-diffs/framework-activity-window-continuation.json"
    ...
    cluster=("window_attached","window_added","window_visible","idle_handler_scheduled","viewroot_handoff")
    target_pass=bool(contract.get("passed") is True and result.get("launch_result")==0 and
        result.get("launch_stage")=="resumed" and after.get("post_resume_completed") is True and
        all(after.get(key) is True for key in cluster) and
        after.get("framework_trace",[])[-1:]==["handoff.viewroot_surface"] and
        result.get("classification")=="viewroot_surface_handoff" and
    ...
    summary={
      ...
      "target":{"name":TARGET,"status":"pass" if target_pass else "fail",
        "stage":"viewroot_surface_handoff" if target_pass else "activity_window_cluster_blocked",
        "normalized_signature":"" if target_pass else "framework:activity_window_cluster:closure_failed",
        ...
        "requirements":{"synthetic_contract":contract.get("passed") is True,
          "real_apk_post_resume":after.get("post_resume_completed") is True,
          "window_cluster":{key:after.get(key) for key in cluster},
          "next_boundary_recorded":result.get("classification")}},
      "coverage_delta":{"new_imports":[],"new_android_api":["Activity.getWindow","Window.getDecorView","View.setVisibility","Activity.getWindowManager","Window.getAttributes","WindowManager.addView"],"new_jni":[],"new_lifecycle":["ActivityThread.handleResumeActivity window/visibility cluster"]},
      "behavior_delta":["generic APK launch completes API19 ActivityThread Window/decor visibility cluster through the ViewRoot/Surface handoff"],
