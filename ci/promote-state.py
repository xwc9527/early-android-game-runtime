...
def main():
 p=argparse.ArgumentParser();p.add_argument("summary");p.add_argument("--merged",action="store_true",required=True);a=p.parse_args()
 ...
 if tree!=c["tested_tree"]:raise SystemExit("merged tree differs from closure tested tree")
 state_name="MERGED"
 closure=json.load(open(ROOT/"ci/governance/closure.json",encoding="utf-8"));closure.update({
  ...
  "merged_commit":head,"merged_tree":tree})
 (ROOT/"ci/governance/closure.json").write_text(json.dumps(closure,indent=2)+"\n",encoding="utf-8")
 state=json.load(open(ROOT/"ci/governance/state.json",encoding="utf-8"));state["baseline"]["commit"]=head;state["baseline"]["last_known_good"]=head;state["active"]["lifecycle"]="MERGED";state["active"]["primary_blocker"]="none";state["active"]["first_broken_boundary"]="none";state["active"]["first_broken_boundary_evidence"]="closure evidence"
 ...
 if c.get("target")=="PVS1":
  for module in modules["modules"]:
   if module["name"] in ("NativeActivityInput",):module["status"]="stable";module["reopen_reason"]=None
 ...
 current=re.sub(r"Baseline: `main @ [0-9a-f]+`",f"Baseline: `main @ {head}`",current)
 current=re.sub(r"Last known good: `[^`]+` on formal `main`",f"Last known good: `{head}` on formal `main`",current)
 ...
 for name in ("NativeActivity/Window","Looper/InputQueue","Simulator regression harness"):
 ...
 print(f"AGR state prepared: {state_name} {head} tree {tree}")
