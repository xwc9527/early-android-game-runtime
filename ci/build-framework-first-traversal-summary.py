#!/usr/bin/env python3
"""Bind API19 first-traversal closure evidence to one exact commit and tree."""
...
SEMANTIC = "ci/governance/semantic-diffs/framework-first-traversal.json"
...
def main():
    ...
    parser.add_argument("--output", default="build/artifacts/framework-first-traversal-run-summary.json")
    ...
    synthetic = contract.get("after_traversal", {})
    target_pass = (
        contract.get("passed") is True
        and contract.get("surface_persistent") is True
        and contract.get("second_traversal_result") == 0
        and result.get("launch_result") == 0
        and result.get("launch_stage") == "resumed"
        and result.get("real_owner_graph") is True
        and result.get("traversal_result") == 0
        and result.get("classification") == "viewroot_surface_ready"
        and all(after.get(key) is True for key in ("hierarchy_attached", "layout_complete", "surface_valid"))
        and after.get("traversal_count") == 1
        and after.get("surface_generation") == 1
        and after.get("measured_width", 0) > 0
        and after.get("measured_height", 0) > 0
        and synthetic.get("surface_valid") is True
    ...
    evidence = {
        ...
        "synthetic_first_traversal_contract": bool(contract),
    ...
    summary = {
        ...
        "target": {
            ...
            "stage": "viewroot_surface_ready" if target_pass else "first_traversal_blocked",
            "normalized_signature": "" if target_pass else "framework:first_traversal:closure_failed",
            ...
            "requirements": {
                ...
                "real_apk_surface_ready": result.get("classification") == "viewroot_surface_ready",
                "persistent_surface": contract.get("surface_persistent") is True,
                "first_traversal_count": after.get("traversal_count"),
            },
        },
        "coverage_delta": {
            "new_imports": [],
            "new_android_api": ["ViewRootImpl.doTraversal", "ViewRootImpl.performTraversals",
                                "View.measure", "IWindowSession.relayout", "View.layout", "Surface.isValid"],
            "new_jni": [],
            "new_lifecycle": ["first root Surface acquisition and traversal reschedule"],
        },
        "behavior_delta": ["generic APK launch reaches a measured, laid-out root with a valid persistent Surface"],
    ...
    print(json.dumps({"classification": classification, "target_pass": target_pass,
                      "tested_commit": head, "tested_tree": tree}))
