...
import io,json,os,pathlib,subprocess,urllib.parse,urllib.request,zipfile
  ...
  for run in runs.get("workflow_runs",[]):
   ...
   for art in arts.get("artifacts",[]):
    ...
    with zipfile.ZipFile(io.BytesIO(bytes_url(art["archive_download_url"]))) as z:
     ...
     if c.get("eligible_for_merge") and c.get("tested_commit")==sha and c.get("tested_tree")==candidate_tree:
