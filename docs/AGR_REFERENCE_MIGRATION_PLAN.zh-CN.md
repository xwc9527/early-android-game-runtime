...
上层 cluster 只在自己的一条 crossing edge 上记录 prerequisite 并停止该 origin 的递归。`external_cluster_sources` 里的 source-derived owner 也可以带 `prerequisite_edges`，但只能匹配该 owner 自己的 `cross_cluster_source_edges`。截断绑定当前 owner、exact crossing 和到达它的 origin；observed entry 的 stop key 不沿递归树传播。例如 `Framework.ZygotePreload` 留在上层 closure，只截断它进入 `Dalvik.ClassInitialization` 和 `Libcore.BootClassLoading` 的边。截断字段必须与该 crossing 的 edge、owner、repo、revision、file、symbol、SHA 一致，禁止凭空声明。`UNRESOLVED` 与 `REOPEN_REQUIRED` 进入 closure work queue，并保留声明该边的 owner 自己的 `origin_dependency_ids` 和完整 `source_paths`。`PREREQUISITE_CLOSED` 只在 exact crossing closure 匹配后才能离开 work queue。记录必须通过 `external_edge_closed`，并且 `ci/governance/substrate-production-contracts.json` 的 `crossing_closures` 里有同一 edge、semantic owner、repo、revision、file、symbol、SHA-256 的 `CROSSING_CLOSED` 记录。该记录绑定声明 origin、source path、由该 path 推导的 production scope、production module、source owner revision/digest、tested commit/tree、从证据重算的 differential，以及 `closure_run`。`closure_run` 必须是 `ci/governance/closure-attempts.json` 中 `target` 等于该记录 `closure_target`、tested commit 与 tested tree 完全一致的唯一一条 `VALID_PASS`，并且 `git rev-parse <tested_commit>^{tree}` 等于该 tree。同名 owner 的 `PRODUCTION_CLOSED` 不能覆盖这条 crossing。scope 短于该 path、混入未到达能力、symbol/hash 不同、origin 或 source path 不同、differential 不匹配，或手写关系，一律拒绝。`contracts` 数组仍是整 owner 列表，保持为空，不能自证整个 substrate owner。production module reopen 仍由 `ci/governance/reopens.json` 治理。没有 CLEAN producer 只拒绝 `PREREQUISITE_CLOSED`，不阻止把真实 crossing 重分类为 `UNRESOLVED`。substrate 的内部依赖属于 substrate 自己的 closure，不计入声明了该边的上层 origin。当前确认的 substrate owner 是 `Libcore.BootClassLoading`、`Dalvik.BootClassResolution`、`Dalvik.ClassInitialization`、`Dalvik.ClassVerification`、`Dalvik.Monitor`、`Dalvik.ThreadState`、`Dalvik.MethodInvocation`、`Dalvik.ObjectAllocation`、`Dalvik.StaticFieldArrayRoots`、`Dalvik.JNINativeBinding`、`Bionic.PthreadCondition`、`Bionic.ClockGettime`。`Framework.ZygotePreload` 仍是 Integer 的启动环境 source owner。`Framework.ZygoteVMOptions` 与 `AndroidNative.InitZygote` 保持现有窄 source owner。新增 substrate owner 必须先确认。

`prerequisite_edges` 是上层 cluster 到公共 substrate 的关系，不是 owner 的 `SOURCE_LOCATED` / `SOURCE_CLOSED` 状态。关系只有 `UNRESOLVED`、`PREREQUISITE_CLOSED`、`REOPEN_REQUIRED`。`PREREQUISITE_CLOSED` 只表示这一条 exact crossing 的 production scope 已关闭，不表示整个 semantic owner 已 `PRODUCTION_CLOSED`。上层仍须完成自己的 source files、inline edges、boundary contracts、零 unresolved edge 和 cluster review，才能 `MIGRATION_AUTHORIZED`。`UNRESOLVED` 与 `REOPEN_REQUIRED` 都阻止上层 `SOURCE_CLOSED` 和 `MIGRATION_AUTHORIZED`。私有 source file、`SERVICE_HLE`、excluded HLE boundary 和 `GAME_PATCH` 不能绕过。公共 substrate 的缺陷必须 evidence-backed REOPEN 该公共 owner，由它单独完成 source repair、differential 和 CLOSE 后，上层再引用。Integer 的六条真实 crossing 现为 `UNRESOLVED`，不写 `PREREQUISITE_CLOSED`：`valueOf` 到 `Dalvik.ClassInitialization`、`Dalvik.ObjectAllocation`、`Dalvik.MethodInvocation`、`Dalvik.StaticFieldArrayRoots`，以及 `Framework.ZygotePreload` 到 `Dalvik.ClassInitialization` 和 `Libcore.BootClassLoading`。`Framework.ZygotePreload` 留在 Integer closure。`Dalvik.StaticFieldArrayRoots` 的 source 状态仍是 `SOURCE_CLOSED`，关系仍是 `UNRESOLVED`。Integer cluster 保持 `SOURCE_LOCATED` 且 `MIGRATION_AUTHORIZED=false`。Resources 只把 `AndroidNative.JNIHelp` 到 `Dalvik.JNINativeBinding` 的 `Dalvik RegisterNatives method binding` 记为 `REOPEN_REQUIRED`，不写 `PREREQUISITE_CLOSED`。依据是 pinned CLEAN image `2d4f0d0a7a193b829349c0f9247b670c600ee2690eeb1f8949fc5794b2639645` 与 Dalvik `36e356c96640775f0a3f167bd2426ea0f0093b8b` `vm/Jni.cpp` `RegisterNatives` / `dvmRegisterJNIMethod`，相对 `Runtime/DexLoom/VM/dx_jni.c` `jni_RegisterNatives` 在 `missing_signature`、`non_native`、`null_fn`、`fast_static` 上的 machine divergence。这条关系记录本身不是 module reopen。`AndroidNative.JNIHelp` 留在 Resources closure。它的两条 blocking edge 保留，因为 source map 把 nativehelper 的 class 解析和 `RegisterNatives` 调用，与 Dalvik 绑定已声明方法分开。Resources cluster 保持 `SOURCE_LOCATED` 且 `MIGRATION_AUTHORIZED=false`。`SOURCE_DERIVED` substrate owner 的 `MIGRATION_AUTHORIZED` 只能由生成器根据 `source_derived_reviews` 推导，provenance 保持 `SOURCE_DERIVED`。review 必须匹配该 owner 的 repository、revision、source file SHA-256、source closure、已审查 dependency/boundary，并且没有 unresolved edge；manifest 必须保留到达它的 origin dependency 和 source path。裸布尔值不能自证。`Bionic.ClockGettime` 的 review 只覆盖当前 `Bionic.PthreadCondition` crossing 的 `CLOCK_REALTIME` 与 `CLOCK_MONOTONIC`，并按路径和 SHA-256 引用 Darwin host-boundary evidence。这不是 `PRODUCTION_CLOSED` 或 `PREREQUISITE_CLOSED`。
