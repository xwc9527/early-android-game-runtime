...
| Framework traversal dispatch / SurfaceView lifecycle | active | — | host SurfaceHolder lock/post contract; Simulator discovery `35683118511` confirmed surfaceCreated; Linux vtable, Java Thread, and canvas lock contracts | a production change to measure, relayout, layout, or root Surface identity follows the first-traversal reopen rule. Virtual dispatch and `Thread.start` ownership stay closed. `lockCanvas` binds the child content Surface. The current stop is unresolved `Canvas.drawBitmap`. That is not a first frame. |
