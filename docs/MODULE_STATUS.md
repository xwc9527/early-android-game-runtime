...
| Framework traversal dispatch / SurfaceView lifecycle | active | — | host SurfaceHolder contract; Simulator discovery `35683118511` confirmed surfaceCreated; Linux vtable inheritance contract | a production change to measure, relayout, layout, or root Surface identity follows the first-traversal reopen rule. Virtual dispatch is repaired. The current stop is synchronous `java.lang.Thread.start`. |
