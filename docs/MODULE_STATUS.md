...
| Framework traversal dispatch / SurfaceView lifecycle | active | — | host SurfaceHolder contract; Simulator discovery `35683118511` confirmed surfaceCreated; Linux vtable and Java Thread contracts | a production change to measure, relayout, layout, or root Surface identity follows the first-traversal reopen rule. Virtual dispatch and `Thread.start` ownership are repaired locally. The current stop is unresolved `SurfaceHolder.lockCanvas`. |
